...
function getGroupColor(group) {
  const statuses = (group.rows || [])
    .map((r, idx) => String(getRowStatus(group.n, idx, r) || "").trim())
    .filter(Boolean);

  if (!statuses.length) return null;

  const hasPriority1 = statuses.includes("계기교체(1순위)");
  const allPriority2 = statuses.every(v => v === "계기교체(2순위)");
  const allModem = statuses.every(v => v === "모뎀설치");

  if (hasPriority1) return "red";
  if (allPriority2) return "yellow";
...
function buildStatusFilter() {
  buildMultiSelect(statusDropdown, "statusFilterCheck", [
    ...
    { value: "yellow", label: "🟡 계기교체(2순위)", checked: false },
...
function updateMarkerCount(items = []) {
  ...
  (items || []).forEach(g => {
    (g.rows || []).forEach((r, idx) => {
      ...
      const rowStatus = String(getRowStatus(g.n, idx, r) || "").trim();
      ...
      if (activeStatuses.includes("yellow") && rowStatus === "계기교체(2순위)") {
        meterCount += 1;
        return;
      }
...
function infoRowsWithIndividualCopy(label, values, copyLabel) {
  ...
  const normalized = rows
    .map(item => ({
      meter: String((item && item.meter) || "").trim(),
      apt: String((item && item.apt) || "").trim(),
      store: String((item && item.store) || "").trim()
    }))
  ...
  normalized.forEach(item => {
    const key = `${item.meter}__${item.apt}__${item.store}`;
    if (!seen.has(key)) {
      seen.add(key);
      unique.push(item);
    }
  ...
  const displayValues = unique.length ? unique : [{ meter: "-", apt: "", store: "" }];

  const htmlValues = displayValues.map(item => {
    const safeMeter = escapeHtml(item.meter);
    const rawMeter = item.meter.replaceAll("'", "\\'");
    return `
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid #f1f5f9;">
        <div style="flex:1;min-width:0;word-break:break-word;">
          <div>${safeMeter}</div>
          ${item.apt ? `<div style="margin-top:2px;color:#4b5563;">공동주택명: ${escapeHtml(item.apt)}</div>` : ``}
          ${item.store ? `<div style="margin-top:2px;color:#4b5563;">상호명: ${escapeHtml(item.store)}</div>` : ``}
        </div>
        <button
          onclick="copyFieldValue('${rawMeter}', '${copyLabel}')"
          style="flex-shrink:0;border:1px solid #d1d5db;background:#fff;color:#111827;padding:4px 8px;border-radius:8px;font-size:11px;cursor:pointer;">
          복사
        </button>
      </div>
    `;
  ...
  return `
    <div style="display:flex;flex-direction:column;gap:4px;margin-bottom:8px;">
      <div style="word-break:break-word;">
        <strong>${label}:</strong>
        <div style="margin-top:4px;padding-left:2px;">${htmlValues}</div>
      </div>
    </div>
  `;
}
...
function drawMarkers(items) {
  ...
  updateMarkerCount(visibleItems);
